...
from core.inference_anchor import get_multi_stage_outputs
from core.inference_anchor import aggregate_results
# from core.inference_anchor6 import group_results
from core.nms_anchor import pose_nms
# from core.nms_anchor import pose_nms as pose_nms_anchor
from core.inference_anchor11 import debug
from core.inference_anchor11 import show_results
    ...
    for i, images in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            for scale in sorted(cfg.TEST.SCALE_FACTOR, reverse=True):
                ...
                angle, posemap = get_multi_stage_outputs(
                    cfg, model, image_resized, cfg.TEST.FLIP_TEST, cfg.DATASET.OFFSET_RADIUS
                )
                poses, reverse_scale,heatmap_np_sum = aggregate_results(
                    cfg, poses, angle, posemap, scale
                )
            ...
            if len(poses[0]) == 0:
                ...
                poses, scores = pose_nms(cfg, poses)
                ...
                #     if scores[i_score] > 0.05:
                #         final_results.append(final_poses[i_score])

                if cfg.TEST.DEBUG:
                    debug(image, heatmap_np_sum, scale=reverse_scale)
                    show_results(image, final_poses, i)
