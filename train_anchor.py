...
from core.loss_anchor11 import MultiLossFactory
from core.trainer_anchor import do_train
...
def main_worker(
    ...
    val_loader, test_dataset = make_test_dataloader(cfg)  # new add for train_val
    logger.info(val_loader.dataset)  # new add for train_val
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        # train one epoch
        val_map = 0
        val_map = do_train(cfg, model, train_loader, val_loader, test_dataset, loss_factory, optimizer, epoch, final_output_dir, tb_log_dir, writer_dict)

        lr_scheduler.step()
        if epoch == 0:
            perf_indicator = 0
        else:
            if epoch < 140 and epoch % 2 != 0:
                perf_indicator = val_map
            else:
